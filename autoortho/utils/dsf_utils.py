...
class DsfUtils:
    ...
    def get_ter_tiles_from_dsf(self, dsf_path: str):
        """ get the ter tiles from the dsf """
        if not os.path.exists(self.dsf_tool_location):
        ...
        command = [self.dsf_tool_location, "--dsf2text", dsf_path, "-"]

        final_ter_tiles = set()
        found_ter_tiles = False
        stopped_early = False

        proc = self._run_silent_subprocess(command)
    ...
    def get_maptype_from_dsf(self, dsf_path: str):
